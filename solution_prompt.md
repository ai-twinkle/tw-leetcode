...
* 當一個迴圈邏輯較為複雜時應當拆解
  ...
  * 不可把空迴圈的宣告視為一個 Step
  * 第一個介紹該迴圈的 Step 必須包含「迴圈中的第一段實際邏輯」，不可只介紹迴圈結構
  * 若展示為部分迴圈程式，後面仍有程式碼未講解時 → 加入 `// ...` 表示該迴圈仍未講解完
  * 若展示為部分迴圈程式時無後續程式碼 → 不加 `// ...`
  * 不可逐行拆解，只能按功能邏輯拆解
  * 同一個迴圈被拆成多個 Step 時：後續 Step 必須「保留相同的外層迴圈骨架」，且在迴圈內以 `// Step X：...` 標記前面已講解的區段；不得在不同 Step 重複展開或改寫同一段已講解的迴圈內容，避免邏輯重複並破壞還原性。
  * 若某段程式碼已在 Step X 中展開，則 Step X 之後不得再次展開該段程式碼，只能以 `// Step X：...` 省略標記表示
...
* 當一個內部函數或外部輔助函數邏輯較為複雜時應當拆解
  ...
  * 不可把空函數的宣告視為一個 Step
  * 第一個介紹該函數的 Step 必須包含函數中的第一段實際邏輯
